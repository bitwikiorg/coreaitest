...
async function run() {
    try {
        ...
        const breadth = parseInt(process.argv[3] ||
            (await askQuestion('Enter research breadth (2-10)? [3] ')), 10) || 3;
        const depth = parseInt(process.argv[4] ||
            (await askQuestion('Enter research depth (1-5)? [2] ')), 10) || 2;
        output.log('\nStarting research...');
